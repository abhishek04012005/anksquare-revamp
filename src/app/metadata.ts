import type { Metadata } from 'next'

export const homepageMetadata: Metadata = {
  title: 'Ank Square – Web Development & Merchant Account Management Services for Your Business Growth',
  description: 'Transform your business with Ank Square’s expert web development and merchant account management services. We build fast, SEO-friendly websites and manage seller accounts on Amazon, Flipkart & more. 5+ years experience, 500+ projects delivered.',
  keywords: [
    'website development',
    'web development services',
    'custom website development',
    'responsive website design',
    'SEO friendly website development',
    'affordable website development services india',
    'small business website developer india',
    'startup website development company india',
    'fast loading website development india',
    'business website development india',
    'affordable website development services worldwide',
    'small business website developer worldwide',
    'startup website development company worldwide',
    'fast loading website development worldwide',
    'business website development worldwide',
    'digital services',
    'digital marketing',
    'SEO services',
    'search engine optimization services india',
    'local SEO services india',
    'technical SEO services',

    'merchant account management',
    'merchant account setup',
    'payment gateway integration india',
    'online payment solutions india',
    'secure payment processing services',

    'Amazon seller account',
    'amazon seller account management services india',
    'amazon account setup service india',
    'amazon product listing optimization india',
    'amazon seller account handling service india',

    'Flipkart seller account',
    'flipkart seller account management services india',
    'flipkart onboarding service for sellers india',
    'flipkart seller growth services india',

    'e-commerce solutions',
    'ecommerce website development india',
    'ecommerce consulting services india',
    'online store development india',
    'shopify website development india',
    'woocommerce development india',

    'online marketplace management',
    'ecommerce account management india',
    'marketplace account management services india',

    'web development company',
    'web development company india',
    'website design company for small business india',
    'freelance web developer india',

    'digital marketing agency',
    'digital marketing agency india',

    'business growth',
    'online business growth services',
    'lead generation website development',
    'conversion optimized website design'
  ],
  openGraph: {
    title: 'Ank Square – Web Development & Merchant Account Management Services for Your Business Growth',
    description: 'Expert merchant account management, custom website development, and digital marketing solutions. 500+ projects completed with 300+ happy clients.',
    url: 'https://www.anksquare.com',
    siteName: 'Ank Square',
    type: 'website',
    locale: 'en_US',
    images: [
      {
        url: '/og-image.jpg',
        width: 1200,
        height: 630,
        alt: 'Ank Square – Web Development & Merchant Account Management Services for Your Business Growth',
      },
    ],
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Ank Square – Web Development & Merchant Account Management Services for Your Business Growth',
    description: 'Expert merchant account management, custom website development, and digital marketing solutions.',
    images: ['/og-image.jpg'],
  },
}